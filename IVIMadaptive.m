function [S,f,D,Dster,SSE,rsquare] = IVIMadaptive(selection,bvec,TR,TE)
...
for k=1:ssel(2)
    try
        ...
        while bs>minbs
            %% first get D
            a=polyfit(bvec(size(bvec,1)-bs+1:end),lts(size(bvec,1)-bs+1:end),1);
            D_temp(bs-minbs)=-a(1);
            ...
            f_temp(bs-minbs)=(fff*aa)/(bb+aa*fff-bb*fff);
            %% restrict the perfusion fraction to possitive values
            if f_temp(bs-minbs)<0
                f_temp(bs-minbs)=0.001;
            end
            ...
            [c21, geof]=fit(double(bvec),double(data1),modelbiexpD,'problem',{double(TR),double(TE), T1, T2, T1b, T2b,double(s0),D_temp(bs-minbs),f_temp(bs-minbs)});
            %                     figure
            %                     semilogy(bvec,data1)
            %                     hold on
            ...
            Dstertemp(bs-minbs)=c21.Dster;
            sse(bs-minbs)=geof.sse;
        ...
        [~, ind]=min(sse);
        f_t=f_temp(ind);
        S_t=bvec(size(bvec,1)-bs+1);
        D_t=D_temp(ind);
        Dster_t=Dstertemp(ind);
        SSE(k)=geof.sse;
