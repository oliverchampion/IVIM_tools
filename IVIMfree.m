function [S,f,D,Dstr,SSE,rsquare] = IVIMfree(selection,bvec,TR,TE,allb)

%%%%%%%%
% Voxel-wise model fitting of IVIM using the T1/T2 componsated IVIM
...
% input matrix "selection". The program uses the IVIM-free aproach from
% the article
%
% Input:
...
% long
%
% the following input are options:
%
...
h=waitbar(0,'precentage in');
for k=1:ssel(2)
    if rem(k,50)==0
        waitbar(k/ssel(2),h,'percentage in')
    end
    try
        bvec=bvecbu;
        data1=selection(:,k);
        ...
        % fitoptions
        sbiexpf=fitoptions('Method','NonlinearLeastSquares','robust','on','Startpoint',[Dcon(1) Dstercon(1) data1(1) fcon(1)],'Lower',[Dcon(2) Dstercon(2) 0.1*data1(1) fcon(2)],'Upper',[Dcon(3) Dstercon(3) 10*data1(1) fcon(3)]);
