function [S,DDC,alpha,SSE,rsquare] = stretchedexp(selection,bvec,allb)

%%%%%%%%
% This program calculates S, DDC, alpha, SSE and rsquare for each voxel in the
% input matrix "selection". The program uses the Stretched-exp aproach from
% the article. Fit model from http://doi.org/10.1002/mrm.10581
%
% Input:
...
for k=1:ssel(2)
    ...
    try
        ...
        % fitoptions
        stret_exp=fitoptions('Method','NonlinearLeastSquares','robust','on','Lower',[DDCcon(2) 0.1*data1(1) alphacon(2)],'Upper',[DDCcon(3) 10*data1(1) alphacon(3)],'Startpoint',[DDCcon(1) data1(1) alphacon(1)],'MaxIter',1000);
        
        % fit model from http://doi.org/10.1002/mrm.10581
        modelstret_exp=fittype('a*exp(-(x*D)^alpha)','options',stret_exp);
        
        % fit
        [c21, gof]=fit(bvec,data1,modelstret_exp);
